...
exports.bookinstance_delete_post = (req, res, next) => {
  BookInstance.findById(req.params.id).exec((err, bookinstance) => {
    if (err) return next(err);
    if (bookinstance == null) return next(new Error("Book Instance not found"));
...
exports.bookinstance_update_get = (req, res, next) => {
  async.parallel({
    ...
    if (result.bookInstance == null) {
      const er = new Error('Instance not found');
      er.status = 404;
      return er;
    }
    const selected_book = result.books.find(book => book._id.toString() == result.bookInstance.book.toString())._id.toString();
    res.render('bookinstance_form', 
    { 
